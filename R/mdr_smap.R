...
#' @description \code{s_map_mdr_all} performs MDR S-map introduced in Chang et al. (2021) Ecology Letters.
...
make_block_mvd <- function (block,
  ...
  if (include_var == "all_significant") {
    ...
    for (i in 1:nrow(uic_res)) {
      block_new <- block[,(uic_res[i,"cause_var"])] %>% dplyr::lag(n = abs(uic_res[i,"tp"])) %>% data.frame
    ...
    # ---------------------------------------------------- #
    uic_res <- uic_res %>% dplyr::group_by(cause_var) %>% dplyr::filter(te == max(te))
    for (i in 1:nrow(uic_res)) {
      block_new <- block[,(uic_res[i,"cause_var"])] %>% dplyr::lag(n = abs(uic_res[i,"tp"])) %>% data.frame
      colnames(block_new) <- sprintf("%s_tp%s", uic_res[i,"cause_var"], uic_res[i,"tp"])
      block_mvd <- cbind(block_mvd, block_new)
