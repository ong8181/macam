...
### `rarefy_even_coverage()`, `rarefy_even_coverage2()`, and `plot_rarefy()`
```{r, eval = FALSE}
# Load library
library(tidyverse)
library(phyloseq)
...
block_mvd <- make_block_mvd(data_4sp_std, uic_res, effect_var, max_lag = 1, include_var = "all_significant", p_threshold = 0.05)

# Step. 4: Compute multiview distance based on embeddings with 3 dimensions
multiview_dist <- compute_mvd(block_mvd, effect_var, E = 3, tp = 1)

# If you do not want to use rEDM package, use the following code
#multiview_dist <- compute_mvd(block_mvd, effect_var, simplex_func = "custom", E = 3, tp = 1)
...
mdr_res$stats # rho = 0.476; rmse = 0.770
