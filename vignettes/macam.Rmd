...
packageVersion("rEDM") # v0.7.5
library(rUIC)
packageVersion("rUIC") # v0.1.5

# Load data
data(data_4sp)
effect_var <- "Girella.punctata"

# Step. 1: Estimate optimal embeding dimension
simp_x <- rUIC::simplex(data_4sp, lib_var = effect_var, E = 0:10, tp = 1)
(Ex <- simp_x[which.min(simp_x$rmse),"E"])

# Step 2: Perform UIC to detect causality
uic_res <- uic_across(data_4sp, effect_var, E_range = 0:10, tp_range = -4:0, random_seed = 1111, silent = TRUE)

# Step 3: Make block to calculate multiview distance
block_mvd <- make_block_mvd(data_4sp, uic_res, effect_var, E_effect_var = Ex, include_var = "all_significant")
...
mdr_res$stats # rho = 0.339; rmse = 28.687

# Compare with the normal S-map
smap_res <- rEDM::block_lnlp(data_4sp, target_column = effect_var,
                 method = "s-map", theta = 0.1, tp = 1)
smap_res$rho # rho = 0.318
smap_res$rmse # rho = 28.360

# An all-in-one wrapper function
all_res <- s_map_mdr_all(data_4sp, effect_var, random_seed = 1111)
## Note that include_var = "tp0_only" is used in s_map_mdr_all()
## And thus, the result is different from the example using step-by-step functions
